 ...
 */
class JacksonJrProcessorSpec extends Specification {
    @Shared
    @AutoCleanup
    ...
    void "test big decimal"() {
        ...
        given:
        JacksonJrProcessor processor = new JacksonJrProcessor()
        ...
        def expectedNode = JacksonJrsTreeCodec.SINGLETON.readTree(new JsonFactory().createParser(string))
        boolean complete = false
        TreeNode node = null
        Throwable error = null
        int nodeCount = 0
        processor.subscribe(new Subscriber<TreeNode>() {
            @Override
    ...
    void "test big decimal - USE_BIG_DECIMAL_FOR_FLOATS"() {
        ...
        given:
        def cfg = GenericDeserializationConfig.DEFAULT.withUseBigDecimalForFloats(true)
        JacksonJrProcessor processor = new JacksonJrProcessor(cfg)
        BigDecimal dec = new BigDecimal("888.7794538169553400000")

        when:
        def string = '{"bd1":"888.7794538169553400000","bd2":888.7794538169553400000}'
        byte[] bytes = string.getBytes(StandardCharsets.UTF_8)
        def expectedNode = cfg.getJrsTreeCodec().readTree(new JsonFactory().createParser(string))
        boolean complete = false
        TreeNode node = null
        Throwable error = null
        int nodeCount = 0
    ...
    void "test big integer - USE_BIG_INTEGER_FOR_INTS"() {
        ...
        given:
        def cfg = GenericDeserializationConfig.DEFAULT.withUseBigIntegerForInts(true)
        JacksonJrProcessor processor = new JacksonJrProcessor(cfg)
        BigInteger bInt = new BigInteger("9223372036854775807")
    ...
    void "test nested arrays"() {
        given:
        JacksonJrProcessor processor = new JacksonJrProcessor(true)
        ...
        complete
        nodeCount == 7
        nodes[0].equals(new JrsNumber(1))
        nodes[1].equals(new JrsNumber(longValue))
        nodes[2].size() == 4
        nodes[3].value.toBigDecimal() == bigDecimalValue
        nodes[4].size() == 3
        nodes[5].equals(new JrsNumber(11))
        nodes[6].equals(new JrsNumber(bigIntegerValue))
    }
